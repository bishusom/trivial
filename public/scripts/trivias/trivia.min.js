let messages={gold:["🏆 Trivia Deity! The knowledge gods bow before you! Can you maintain your reign?","🧠 Mind = Blown! Think you can top this perfect score? Try again!","🤯 Unstoppable Genius! Ready for an even bigger challenge next round?","🎖️ Absolute Legend! The leaderboard needs your name again!"],silver:["✨ Brainiac Alert! One more round could push you to perfection!","🚀 Knowledge Rocket! You're just one launch away from trivia greatness!","💎 Diamond Mind! Polish your skills further with another game!","🧩 Puzzle Master! Can you complete the picture perfectly next time?"],bronze:["👍 Solid Effort! Your next attempt could be your breakthrough!","📚 Bookworm Rising! Every replay makes you wiser - try again!","💡 Bright Spark! Your knowledge is growing - fuel it with another round!","🏅 Contender Status! The podium is within reach - one more try!"],zero:["💥 Knowledge Explosion Incoming! Stick around - the next attempt will be better!","🎯 Fresh Start! Now that you've warmed up, the real game begins!","🔥 Fueling Curiosity! Your learning journey starts here - play again!","🚀 Launch Pad Ready! First attempts are just practice - try for real now!","🌱 Seeds of Knowledge Planted! Water them with another try!"],default:["🌱 Sprouting Scholar! Every replay makes you stronger - continue your journey!","🦉 Wise Owl in Training! The more you play, the wiser you become!","📖 Chapter 1 Complete! Turn the page to your next knowledge adventure!","🧭 Learning Compass Active! Your next game could be your true north!"]},els={game:()=>document.querySelector(".game-screen"),summary:()=>document.querySelector(".summary-screen"),question:()=>document.getElementById("question"),options:()=>document.getElementById("options"),nextBtn:()=>document.getElementById("next-btn"),score:()=>document.getElementById("score"),questionTimer:()=>document.getElementById("question-timer"),totalTimer:()=>null,highscores:()=>document.querySelector(".highscores"),highscoresList:()=>document.getElementById("highscores-list"),resultMessage:()=>document.getElementById("result-message"),correctCount:()=>document.getElementById("correct-count"),timeUsed:()=>document.getElementById("time-used"),questionCounter:()=>document.getElementById("question-counter"),difficultyDisplay:()=>document.getElementById("difficulty-level-display"),difficultyBox:()=>document.querySelector(".difficulty-box")},timers={quick:30,long:60},audio={tick:document.getElementById("tick-sound"),correct:document.getElementById("correct-sound"),wrong:document.getElementById("wrong-sound")},imageCache=(audio.tick.loop=!0,{}),state={isMuted:!1,questions:[],current:0,score:0,timeLeft:null,timerId:null,highScores:[],answers:[],isScoreSaved:!1,isNextPending:!1,selectedQuestions:10,usedQuestions:new Set,fbUsedQuestions:JSON.parse(localStorage.getItem("fbUsedQuestions")||"[]"),fbUsedQuizIds:JSON.parse(localStorage.getItem("fbUsedQuizIds")||"[]"),isTimedMode:!0,timerDuration:"long",difficulty:"mixed",rememberSettings:!1,showSettingsOnStart:!0},nlp,QUIZ_TYPES=("undefined"!=typeof window&&window.nlp?(nlp=window.nlp,console.log("Found nlp")):(nlp={nouns:()=>({out:()=>[]}),adjectives:()=>({out:()=>[]})},console.log("Creating home-grown nlp")),{DAILY:"daily",WEEKLY:"weekly",MONTHLY:"monthly"}),CACHE={QUESTIONS:"trivia-questions-v1",EXPIRY:864e5};function trackEvent(e,t,s,a){"undefined"!=typeof gtag&&gtag("event",e,{event_category:t,event_label:s,value:a})}function toggleClass(e,t,s){e?.classList?.[t]?.(s)}function playSound(t){state.isMuted?console.log(`Sound ${t} skipped: muted`):(console.log("Playing sound: "+t),audio[t].play().catch(e=>console.error(`Error playing ${t} sound:`,e)))}function stopSound(e){console.log("Stopping sound: "+e),audio[e].pause(),audio[e].currentTime=0}function stopAllSounds(){console.log("Stopping all sounds"),["tick","correct","wrong"].forEach(e=>stopSound(e))}function loadMuteState(){state.isMuted=JSON.parse(localStorage.getItem("triviaMasterMuteState"))||!1;var e=localStorage.getItem("triviaMasterTimedMode"),e=(state.isTimedMode=null===e||JSON.parse(e),localStorage.getItem("triviaMasterTimerDuration")),e=(state.timerDuration=null!==e?e:"long",localStorage.getItem("triviaMasterDifficulty")),e=(state.difficulty=null!==e?e:"mixed",document.querySelector("#mute-btn .material-icons"));e&&(e.textContent=state.isMuted?"volume_off":"volume_up"),state.isMuted&&stopAllSounds()}function showError(e){let t=document.getElementById("error-message");t||((t=document.createElement("div")).id="error-message",t.className="error-message",t.innerHTML=`<div id="error-text">${e}</div><button id="retry-btn" class="btn small primary">Retry</button>`,els.game().appendChild(t),document.getElementById("retry-btn").addEventListener("click",()=>{initTriviaGame(state.questions[0]?.category||"general knowledge")})),t.classList.remove("hidden")}function getWeekNumber(e){var e=new Date(e),t=(e.setHours(0,0,0,0),e.setDate(e.getDate()+3-(e.getDay()+6)%7),new Date(e.getFullYear(),0,4));return 1+Math.round(((e-t)/864e5-3+(t.getDay()+6)%7)/7)}async function updatePlayerCount(e){try{var t=db.collection("playerCounts").doc(e),s=await t.get(),a=(s.exists?s.data().count:0)+1;await t.set({count:a,lastUpdated:firebase.firestore.FieldValue.serverTimestamp(),category:e},{merge:!0}),console.log("Updating player count: ",a)}catch(e){return console.error("Error updating player count:",e),Math.floor(40*Math.random())+100}}function extractKeywordNLP(e){var t=nlp(e),s=t.nouns().out("array");return 0<s.length||0<(s=t.adjectives().out("array")).length?s[0]:(t=e.replace("?","").split(" ")).find(e=>3<e.length)||t[0]}async function fetchImage(e){if(imageCache[e])return imageCache[e];console.log("Fetching image for ",e);try{var t=await fetch("/.netlify/functions/get-image?keyword="+encodeURIComponent(e));if(!t.ok)throw new Error("HTTP error! status: "+t.status);var s,a=t.headers.get("content-type");if(a&&a.includes("application/json"))return(s=await t.json()).url?(imageCache[e]=s.url,s.url):null;throw new Error("Invalid content type")}catch(e){return console.error("Error fetching image:",e),null}}async function fetchQuestions(e){try{return console.log("Fetching questions for category:",e),[QUIZ_TYPES.DAILY,QUIZ_TYPES.WEEKLY,QUIZ_TYPES.MONTHLY].includes(e.toLowerCase())?await fetchfbQuiz(e.toLowerCase()):await fetchfbQuestions(e,state.selectedQuestions,state.difficulty)}catch(e){throw console.error("Error in fetchQuestions:",e),showError(e.message||"Failed to load questions."),e}}async function fetchfbQuiz(t){if("daily"===t){let e=new Date;let t="fb-daily-quiz-"+e.toISOString().split("T")[0];var i=new Date;i.setHours(24,0,0,0);let s=JSON.parse(localStorage.getItem("fbdailyQuizCache"))?.[t];if(s&&e<i)return shuffle(s.questions);let a=(await db.collection("basic_intro_questions").where("randomIndex",">=",Math.floor(900*Math.random())).where("difficulty","==",state.difficulty).orderBy("randomIndex").limit(5).get()).docs.map(e=>({id:e.id,question:decodeHTML(e.data().question),correct:decodeHTML(e.data().correct_answer),options:shuffle([...e.data().incorrect_answers.map(decodeHTML),decodeHTML(e.data().correct_answer)]),category:"daily",difficulty:e.data().difficulty||"medium",titbits:e.data().titbits||"",image_keyword:e.data().image_keyword||null})).slice(0,5);return localStorage.setItem("fbdailyQuizCache",JSON.stringify({...JSON.parse(localStorage.getItem("fbdailyQuizCache")||"{}"),[t]:{questions:a,timestamp:e.getTime()}})),shuffle(a)}let e=new Date;i=t===QUIZ_TYPES.WEEKLY?`week-${getWeekNumber(e)}-`+e.getFullYear():`month-${e.getMonth()+1}-`+e.getFullYear();let s="fb-quiz-"+i,a=JSON.parse(localStorage.getItem("fbQuizCache"))?.[s];if(a&&Date.now()-a.timestamp<CACHE.EXPIRY&&!state.fbUsedQuizIds.includes(s))return shuffle(a.questions);i=await db.collection("quizzes").doc(t.toLowerCase()).collection("periods").doc(i).get();if(!i.exists)throw new Error(`No ${t} quiz available.`);let o=i.data().questions.map(e=>({id:CryptoJS.MD5(e.question+e.correct_answer).toString(),question:e.question,correct:e.correct_answer,options:shuffle([...e.incorrect_answers,e.correct_answer]),category:t,subcategory:e.subcategory||"",difficulty:e.difficulty||"medium",titbits:e.titbits||"",image_keyword:e.image_keyword||null}));return localStorage.setItem("fbQuizCache",JSON.stringify({...JSON.parse(localStorage.getItem("fbQuizCache")||"{}"),[s]:{questions:o,timestamp:Date.now()}})),state.fbUsedQuizIds.push(s),localStorage.setItem("fbUsedQuizIds",JSON.stringify(state.fbUsedQuizIds)),shuffle(o)}async function fetchfbQuestions(s,a=10,e="mixed"){var t=s.toLowerCase(),i=(console.log("Fetching questions for category:",s,"difficulty:",e),"mixed"===e?["easy","medium","hard"]:[e]),t=`fb_questions-${t}-`+e,o=JSON.parse(localStorage.getItem("fbQuestionsCache"))?.[t];if(o&&o.questions.length>=a&&o.questions.filter(e=>!state.fbUsedQuestions.includes(e.id)).length>=a)return console.log("Using cached questions:",o.questions),processfbQuestions(o.questions,a);var n=[];for(let t of i){let e=db.collection("triviaMaster").doc("questions").collection("items");var r=await(e=(e="general knowledge"!==s?e.where("category","==",s):e).where("randomIndex",">=",Math.floor(900*Math.random())).where("difficulty","==",t).orderBy("randomIndex").limit(2*Math.ceil(a/i.length))).get();r.empty||n.push(...r.docs.map(e=>({id:e.id,question:decodeHTML(e.data().question),correct:decodeHTML(e.data().correct_answer),options:shuffle([...e.data().incorrect_answers.map(decodeHTML),decodeHTML(e.data().correct_answer)]),category:e.data().category,subcategory:e.data().subcategory||"",difficulty:e.data().difficulty||t,titbits:e.data().titbits||"",image_keyword:e.data().image_keyword||null})))}return 0===n.length?(console.warn(`No questions found for ${s}, falling back to general knowledge`),showError(`No questions available for ${s}. Loading general knowledge instead.`),fetchfbQuestions("general knowledge",a,e)):(console.log("Fetched questions:",n),localStorage.setItem("fbQuestionsCache",JSON.stringify({...JSON.parse(localStorage.getItem("fbQuestionsCache")||"{}"),[t]:{questions:n,timestamp:Date.now()}})),processfbQuestions(shuffle(n),a))}function processfbQuestions(e,t){e=e.filter(e=>!state.fbUsedQuestions.includes(e.id)),t=shuffle(e).slice(0,Math.min(t,e.length));return state.fbUsedQuestions.push(...t.map(e=>e.id)),localStorage.setItem("fbUsedQuestions",JSON.stringify(state.fbUsedQuestions.slice(-500))),t.map(e=>({...e,options:shuffle([...e.options]),image_keyword:e.image_keyword||null}))}function decodeHTML(e){var t=document.createElement("textarea");return t.innerHTML=e,t.value}function shuffle(e){var t=[...e];for(let e=t.length-1;0<e;e--){var s=Math.floor(Math.random()*(e+1));[t[e],t[s]]=[t[s],t[e]]}return t}function toInitCaps(e){return e.replace(/\w\S*/g,e=>e.charAt(0).toUpperCase()+e.substr(1).toLowerCase())}function updateDifficultyDisplay(){var e=els.difficultyDisplay(),t=els.difficultyBox();if(e&&t)switch(t.classList.remove("easy","medium","hard","mixed"),state.difficulty){case"easy":e.textContent="Easy",t.classList.add("easy");break;case"medium":e.textContent="Medium",t.classList.add("medium");break;case"hard":e.textContent="Hard",t.classList.add("hard");break;default:e.textContent="Mixed",t.classList.add("mixed")}}function updateTimerUI(){state.timeLeft=state.isTimedMode?timers[state.timerDuration]:null;var e=els.questionTimer()?.parentElement;state.isTimedMode?(toggleClass(e,"remove","hidden"),els.questionTimer()&&(els.questionTimer().textContent=state.timeLeft)):(toggleClass(e,"add","hidden"),els.questionTimer()&&(els.questionTimer().textContent="N/A"))}function initTriviaGame(e){loadMuteState(),state.rememberSettings=JSON.parse(localStorage.getItem("triviaMasterRememberSettings"))||!1,state.showSettingsOnStart=!state.rememberSettings&&"daily"!==e,"daily"===e?(state.isTimedMode=!0,state.timerDuration="quick",state.selectedQuestions=5,state.difficulty="mixed",localStorage.setItem("triviaMasterTimedMode","true"),localStorage.setItem("triviaMasterTimerDuration","quick"),messages.zero=["Don't worry! These were starter questions.","Everyone starts somewhere! Try again?","Basic knowledge builds up over time!","Ready for another quick try?"]):(state.isTimedMode=JSON.parse(localStorage.getItem("triviaMasterTimedMode"))||!0,state.timerDuration=localStorage.getItem("triviaMasterTimerDuration")||"long",state.rememberSettings=JSON.parse(localStorage.getItem("triviaMasterRememberSettings"))||!0,state.difficulty=localStorage.getItem("triviaMasterDifficulty")||"mixed"),els.game().classList.add("active"),els.game().classList.remove("hidden"),els.summary().classList.remove("active"),console.log("Initializing game for category:",e),trackEvent("game_start","game",e,1),state.questions=[],state.current=0,state.score=0,state.answers=[],state.isScoreSaved=!1,state.isNextPending=!1,updateTimerUI(),els.score().textContent="0",updatePlayerCount(e),state.showSettingsOnStart?showSettingsPanel(!0):startGameWithSettings(e),setupEvents()}async function showQuestion(){toggleClass(document.querySelector(".app-footer"),"add","hidden"),els.question().classList.remove("correct-bg","wrong-bg");let s=state.questions[state.current];if(!s)return endGame();var t="general knowledge"===s.category?"general knowledge":toInitCaps(s.category);els.question().innerHTML='<div class="question-loading">Loading question...</div>';try{var a=s.image_keyword||extractKeywordNLP(s.question),i=await fetchImage(a);let e="";i&&(e+=`
        ...
        `}els.options().innerHTML=s.options.map((e,t)=>`<button style="animation-delay: ${.1*t}s" data-correct="${e===s.correct}">${e}</button>`).join(""),els.questionCounter()&&(els.questionCounter().textContent=state.current+1+"/"+state.selectedQuestions),toggleClass(els.game(),"add","active"),toggleClass(els.summary(),"remove","active"),els.questionTimer().textContent=state.isTimedMode?state.timeLeft:"N/A",setupOptionEvents(),startTimer()}function setupOptionEvents(){var e=els.options();e?(e.querySelectorAll("button").forEach(e=>{e.removeEventListener("click",handleOptionClick),e.addEventListener("click",handleOptionClick)}),els.nextBtn().classList.remove("visible")):console.error("Options container (#options) not found in DOM")}function handleOptionClick(e){console.log("Option button clicked:",e.target.textContent,"data-correct:",e.target.dataset.correct),checkAnswer("true"===e.target.dataset.correct)}function startTimer(){clearInterval(state.timerId),state.isTimedMode?(state.timeLeft=timers[state.timerDuration],els.questionTimer().textContent=state.timeLeft,state.timerId=setInterval(()=>{--state.timeLeft<=0&&(clearInterval(state.timerId),handleTimeout()),els.questionTimer().classList.add("urgent"),state.timeLeft<=5&&playSound("tick"),els.questionTimer().textContent=state.timeLeft},1e3),state.isMuted||playSound("tick")):els.questionTimer().textContent="N/A"}function handleTimeout(){state.isTimedMode&&(els.question().classList.add("wrong-bg"),checkAnswer(!1))}function checkAnswer(e){stopSound("tick"),playSound(e?"correct":"wrong"),state.answers.push({correct:e}),clearInterval(state.timerId),els.question().classList.remove("correct-bg","wrong-bg"),els.question().classList.add(e?"correct-bg":"wrong-bg"),els.options().querySelectorAll("button").forEach(e=>{e.disabled=!0,e.classList.add("true"===e.dataset.correct?"correct":"wrong")}),e&&state.isTimedMode&&(state.score+=10*state.timeLeft),els.score().textContent=state.score;var e=state.questions[state.current],t=els.question().querySelector(".question-titbits");t&&t.remove(),e.titbits&&((t=document.createElement("div")).className="question-titbits",t.innerHTML='<span class="titbits-icon">💡</span> Fun Fact: '+e.titbits,els.question().appendChild(t)),state.current<state.selectedQuestions-1?(els.nextBtn().classList.add("visible"),state.isNextPending=!0,state.isTimedMode&&setTimeout(()=>{state.isNextPending&&handleNextQuestion()},2e3)):setTimeout(endGame,1e3)}function handleNextQuestion(){state.isNextPending&&(state.isNextPending=!1,els.nextBtn().classList.remove("visible"),(state.current++<state.selectedQuestions-1?showQuestion:endGame)())}async function endGame(){console.log("Starting endGame, answers:",state.answers.length,"questions:",state.selectedQuestions);trackEvent("game_complete","performance",`${state.answers.filter(e=>e.correct).length}/${state.selectedQuestions} correct`,state.score);var t=els.game(),s=els.summary(),a=els.highscores();if(t&&s&&a){t.classList.add("hidden"),s.classList.add("active"),a.classList.remove("hidden"),document.querySelector(".app-footer")?.classList.remove("hidden"),clearInterval(state.timerId),stopAllSounds();var i=state.questions[0]?.category||"general knowledge";let e=null;try{var o=await db.collection("scores").where("category","==",i).orderBy("score","desc").limit(1).get();e=o.empty?null:o.docs[0].data()}catch(e){console.error("endGame: Error fetching global high score:",e)}try{saveHighScore()}catch(e){console.error("endGame: Error in saveHighScore:",e)}try{await showSummary(e)}catch(e){console.error("endGame: Error in showSummary:",e)}"daily"===state.questions[0]?.category?(i=document.querySelector(".action-buttons"))&&(i.innerHTML=`
                ...
                </div>`,(o=document.getElementById("upgrade-btn"))&&o.addEventListener("click",()=>{state.isTimedMode=!1,state.selectedQuestions=10,localStorage.removeItem("triviaMasterTimedMode"),localStorage.removeItem("triviaMasterTimerDuration"),initTriviaGame("general knowledge")}),updateDailyResetTimer(),setInterval(updateDailyResetTimer,1e3)):(i=document.querySelector(".action-buttons"))&&(i.innerHTML=`
                <button class="btn primary" id="restart-btn">
                    <span class="material-icons">replay</span>Play Again
                </button>
               <a href="/" class="btn primary" style="margin-top: 1rem; max-width: 300px; margin-left: auto; margin-right: auto;">
                    <span class="material-icons">home</span> Back Home
                </a>`,o=document.getElementById("restart-btn"))&&o.addEventListener("click",restartGame)}else console.error("DOM elements missing:",{gameScreen:!!t,summaryScreen:!!s,highscores:!!a})}function updateDailyResetTimer(){var e=new Date,t=new Date,t=(t.setHours(24,0,0,0),t-e),e=Math.floor(t/36e5),t=Math.floor(t%36e5/6e4),s=document.getElementById("daily-reset-timer");s&&(s.textContent=e+`h ${t}m`)}function showSettingsPanel(e){var t=document.getElementById("settings-panel"),s=document.getElementById("timed-mode"),a=document.getElementById("mode-label"),i=document.getElementById("timer-duration"),o=document.getElementById("difficulty-level"),n=document.getElementById("remember-settings");t&&s&&a&&i&&o&&n&&(s.checked=state.isTimedMode,a.textContent=state.isTimedMode?"Timed":"Non-Timed",i.value=state.timerDuration,i.parentElement.style.display=state.isTimedMode?"flex":"none",o.value=state.difficulty||"mixed",n.checked=state.rememberSettings,n.checked=state.rememberSettings||!0,t.classList.remove("hidden"),s=document.getElementById("close-settings-btn"),e&&s?(s.disabled=!0,s.style.opacity="0.5",s.style.cursor="not-allowed"):s&&(s.disabled=!1,s.style.opacity="1",s.style.cursor="pointer"))}function startGameWithSettings(e){fetchQuestions(e).then(e=>{state.questions=e,showQuestion()}).catch(e=>{console.error("Error fetching questions:",e),showError(e.message||"Failed to load questions.")})}function restartGame(){trackEvent("restart_game","navigation","from_summary"),state.usedQuestions.clear(),state.questions=[],state.current=0,state.score=0,state.answers=[],updateTimerUI(),els.score().textContent="0",els.game().classList.remove("hidden"),els.game().classList.add("active"),els.summary().classList.remove("active"),els.highscores().classList.add("hidden"),localStorage.removeItem(CACHE.QUESTIONS),initTriviaGame(state.questions[0]?.category||"general knowledge")}async function showSummary(e){console.log("Starting showSummary, globalHigh:",e);var t=state.isTimedMode?state.selectedQuestions*timers[state.timerDuration]-state.timeLeft:0,s=state.answers.filter(e=>e.correct).length,a=state.questions[0]?.category||"general knowledge";let i,o;o=0===s?i="zero":s>=.9*state.selectedQuestions?i="gold":s>=.7*state.selectedQuestions?i="silver":i="bronze";var n,r,l=messages[i][Math.floor(Math.random()*messages[i].length)],c=els.highscoresList();c?((n=document.querySelector(".performance-message"))&&n.classList.add(o),c.innerHTML='<div class="loading-indicator"><div class="loading-spinner"></div><p>Loading high scores...</p></div>',n=els.correctCount(),c=els.timeUsed(),r=els.resultMessage(),n&&c&&r?(n.textContent=s+"/"+state.selectedQuestions,c.textContent=state.isTimedMode?`${Math.floor(t/60)}m ${(t%60).toString().padStart(2,"0")}s`:"N/A",r.innerHTML=l,e&&r.insertAdjacentHTML("afterend",`
        ...
        `).join(""):'<p class="no-scores">No high scores yet</p>',(s=document.querySelector(".action-buttons"))&&("daily"===state.questions[0]?.category?(s.innerHTML=`
                <div class="progress-message">
                    <p>You've completed today's daily challenge!</p>
                    <p>New questions in <span id="daily-reset-timer"></span></p>
                    <button id="upgrade-btn" class="btn primary">Try Regular Quiz</button>
                ...
                </div>`,(t=document.getElementById("upgrade-btn"))&&t.addEventListener("click",()=>{state.isTimedMode=!1,state.selectedQuestions=10,localStorage.removeItem("triviaMasterTimedMode"),localStorage.removeItem("triviaMasterTimerDuration"),initTriviaGame("general knowledge")}),updateDailyResetTimer(),setInterval(updateDailyResetTimer,1e3)):(s.innerHTML=`
                <button class="btn primary" id="restart-btn">
                    <span class="material-icons">replay</span>Play Again
                </button>
                <a href="/" class="btn primary" style="margin-top: 1rem; max-width: 300px; margin-left: auto; margin-right: auto;">
                    <span class="material-icons">home</span> Back Home
                </a>`,(l=document.getElementById("restart-btn"))&&l.addEventListener("click",restartGame))),els.highscores().classList.remove("hidden"),localStorage.setItem("gamesPlayed",parseInt(localStorage.getItem("gamesPlayed")||"0")+1),await updateHighScores()):console.error("Summary elements missing:",{correctCount:!!n,timeUsed:!!c,resultMessage:!!r})):console.error("Highscores list (#highscores-list) not found in DOM")}async function saveHighScore(){var e,t;!state.isScoreSaved&&state.score&&(e=state.questions[0]?.category||"general knowledge",t=prompt("Enter your name:","Anonymous")||"Anonymous",await db.collection("scores").add({name:t,score:state.score,category:e,difficulty:state.difficulty,timestamp:firebase.firestore.Timestamp.now()}),state.isScoreSaved=!0,updateHighScores())}async function updateHighScores(){var t=els.highscoresList();if(t){var e=state.questions[0]?.category||"general knowledge";try{var s=await db.collection("scores").where("category","==",e).where("difficulty","==",state.difficulty).orderBy("score","desc").limit(5).get();state.highScores=s.docs.map(e=>e.data()),t.innerHTML=state.highScores.length?state.highScores.map((e,t)=>`
                <div class="highscore-entry">
                    <span class="rank">${t+1}.</span>
                    <span class="name">${e.name}</span>
                    <span class="score">${e.score}</span>
                </div>
            `).join(""):'<p class="no-scores">No high scores yet for this category. Play a game to start!</p>'}catch(e){console.error("updateHighScores: Error fetching global high scores:",e),t.innerHTML='<p class="no-scores">Error loading high scores. Please try again later.</p>'}}}function setupEvents(){var e=document.getElementById("next-btn");e&&e.addEventListener("click",handleNextQuestion);let t=document.getElementById("mute-btn");t&&t.addEventListener("click",()=>{state.isMuted=!state.isMuted,localStorage.setItem("triviaMasterMuteState",state.isMuted);var e=t.querySelector(".material-icons");e&&(e.textContent=state.isMuted?"volume_off":"volume_up"),state.isMuted?stopAllSounds():els.game().classList.contains("active")&&state.isTimedMode&&playSound("tick")});e=document.getElementById("settings-btn");let i=document.getElementById("settings-panel"),o=document.getElementById("timed-mode"),s=document.getElementById("mode-label");var a=document.getElementById("save-settings-btn"),n=document.getElementById("close-settings-btn");let r=document.getElementById("timer-duration"),l=document.getElementById("difficulty-level");a&&a.addEventListener("click",()=>{var e=o?o.checked:state.isTimedMode,t=r?r.value:state.timerDuration,s=l?l.value:state.difficulty,a=document.getElementById("remember-settings")?.checked||!1;state.isTimedMode=e,state.timerDuration=t,state.difficulty=s,state.rememberSettings=a,localStorage.setItem("triviaMasterTimedMode",state.isTimedMode),localStorage.setItem("triviaMasterTimerDuration",state.timerDuration),localStorage.setItem("triviaMasterDifficulty",state.difficulty),localStorage.setItem("triviaMasterRememberSettings",state.rememberSettings),updateDifficultyDisplay(),toggleClass(i,"add","hidden"),state.showSettingsOnStart?(state.showSettingsOnStart=!1,startGameWithSettings(state.questions[0]?.category||"general knowledge")):(state.current=0,updateTimerUI(),showQuestion())}),e&&i&&e.addEventListener("click",()=>{toggleClass(i,"remove","hidden"),o&&(o.checked=state.isTimedMode),s&&(s.textContent=state.isTimedMode?"Timed":"Non-Timed"),r&&(r.value=state.timerDuration,r.parentElement.style.display=state.isTimedMode?"flex":"none"),l&&(l.value=state.difficulty)}),o&&o.addEventListener("change",()=>{s.textContent=o.checked?"Timed":"Non-Timed",r&&(r.parentElement.style.display=o.checked?"flex":"none")}),a&&a.addEventListener("click",()=>{var e=o?o.checked:state.isTimedMode,t=r?r.value:state.timerDuration,s=l?l.value:state.difficulty;e===state.isTimedMode&&t===state.timerDuration&&s===state.difficulty||(state.isTimedMode=e,state.timerDuration=t,state.difficulty=s,localStorage.setItem("triviaMasterTimedMode",state.isTimedMode),localStorage.setItem("triviaMasterTimerDuration",state.timerDuration),localStorage.setItem("triviaMasterDifficulty",state.difficulty),state.current=0,updateTimerUI(),showQuestion()),toggleClass(i,"add","hidden")}),n&&n.addEventListener("click",()=>{toggleClass(i,"add","hidden")})}function setupStartQuizListener(){document.addEventListener("startQuiz",e=>{var{category:e,isTimedMode:t}=e.detail;state.isTimedMode=t,initTriviaGame(e)})}document.addEventListener("DOMContentLoaded",()=>{var e;setupStartQuizListener(),loadMuteState(),window.location.pathname.includes("catalog.html")||initTriviaGame(1<(e=window.location.pathname.split("/").filter(e=>e)).length?e[1].replace(/-/g," "):"general knowledge")});export{initTriviaGame};
